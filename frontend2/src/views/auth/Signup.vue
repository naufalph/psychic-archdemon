<template>
  <div class="min-h-screen bg-[#F4F5F7] flex items-center justify-center p-6">
    ...
    <div class="w-full max-w-5xl relative z-10">
      <div v-if="currentStep === 1" v-motion :initial="{ opacity: 0, y: 20 }" :enter="{ opacity: 1, y: 0 }" class="text-center mb-12">
      ...
      <div v-if="currentStep === 1" class="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        ...
        >
          <div class="w-16 h-16 bg-[#F4F5F7] rounded-2xl flex items-center justify-center mb-6 group-hover:bg-[#C5A17A]/10 transition-colors">
        ...
        >
          <div class="w-16 h-16 bg-[#F4F5F7] rounded-2xl flex items-center justify-center mb-6 group-hover:bg-[#7C4728]/10 transition-colors">
      ...
      <div v-if="currentStep === 2" v-motion :initial="{ opacity: 0, y: 20 }" :enter="{ opacity: 1, y: 0 }" class="bg-white rounded-3xl shadow-2xl p-12 max-w-2xl mx-auto border border-gray-100">
      ...
      <div v-if="currentStep === 3" v-motion :initial="{ opacity: 0, scale: 0.9 }" :enter="{ opacity: 1, scale: 1 }" class="bg-white rounded-3xl shadow-2xl p-16 max-w-2xl mx-auto text-center border border-gray-100 relative overflow-hidden">
        <ConfettiExplosion
          v-if="showConfetti"
          :particleCount="100"
          :force="0.3"
...
const validateForm = () => {
  ...
  if (!formData.value.password) {
    ...
    errors.value.password = 'Password must contain at least one digit, one lowercase, one uppercase, and one special character (@#$%^&+=!_-)'
    isValid = false
  }
...
const handleSignup = async () => {
  ...
  try {
    ...
    localStorage.setItem('pending_user_role', selectedRole.value)
    localStorage.setItem('pending_user_email', formData.value.email)
