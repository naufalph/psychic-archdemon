<template>
  <div class="min-h-full bg-[#F4F5F7] px-6 py-8">
    <div class="max-w-lg mx-auto space-y-4">
      ...
      <!-- Role switching -->
      <div v-if="canSwitchToArchitect" class="bg-white rounded-xl border border-gray-200 px-5 py-4">
        <p class="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Role</p>
        <button
          @click="switchToArchitect"
          class="flex items-center gap-3 text-sm font-medium text-gray-700 hover:text-black transition"
        >
          <UserCog :size="16" class="text-gray-500" />
...
import { ChevronRight, LogOut, UserCog } from 'lucide-vue-next'
...
const canSwitchToArchitect = computed(() => authStore.hasRole('ARCHITECT'))
